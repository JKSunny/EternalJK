...
void vk_create_shader_modules( void )
{
    ...
    if ( vk.ssaoActive )
    {
        ...
        vk.shaders.ssao_debug_fs = SHADER_MODULE(ssao_debug_frag_spv);
        ...
        VK_SET_OBJECT_NAME(vk.shaders.ssao_debug_fs, "ssao debug post-processing fragment module", VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT);
    }
#endif
}
...
void vk_destroy_shader_modules( void )
{
    ...
    if ( vk.ssaoActive )
    {
        ...
        qvkDestroyShaderModule(vk.device, vk.shaders.ssao_debug_fs, NULL);
    }
#endif
}