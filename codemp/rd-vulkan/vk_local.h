...
#define MAX_ATTACHMENTS_IN_POOL			( 9 + ( ( 1 + VK_NUM_BLUR_PASSES * 2 ) * 2 ) + 1 ) + 1 + (3) // (6+3=9: cubemap.msaa + cubemap.resolve + cubemap.depth) + refraction_extract + (depth_extract + ssao_extract + ssao_blur)
...
typedef struct {
	...
	struct {
		struct {
			...
			VkSubpassDependency external_to_color;
			VkSubpassDependency color_to_external;
		} subpass_deps;
