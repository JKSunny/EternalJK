...
#ifdef USE_VK_IMGUI
static void vk_create_inspector_renderpass(void)
{
    vkRenderPass_t *rp = &vk.render_pass.inspector;
    Com_Memset( rp, 0, sizeof(vkRenderPass_t) );
...
#ifdef USE_VK_IMGUI
    vk_create_inspector_renderpass();
#endif
}
...
#ifdef USE_VK_PBR
    vk_destroy_render_pass( &vk.render_pass.postfx.blend );
    vk_destroy_render_pass( &vk.depth.extract.render_pass );
...
#ifdef USE_VK_SSAO
    vk_destroy_render_pass( &vk.render_pass.ssao.extract );
    vk_destroy_render_pass( &vk.render_pass.ssao.blur );
#endif
#endif

#ifdef USE_VK_IMGUI
    vk_destroy_render_pass( &vk.render_pass.inspector );
#endif
}
