...
set tmpf=spirv\data_pixel.spv
set outf=+spirv\shader_data_pixel.c
set outfb=+spirv\shader_binding_pixel.c

echo %bh%

mkdir spirv

del /Q spirv\shader_data_pixel.c
del /Q spirv\shader_binding_pixel.c
del /Q "%tmpf%"
...
"%bs%" %outfb% "static void vk_set_shader_name_pixel( VkShaderModule shader, const char *name ) {"
"%bs%" %outfb% "    VK_SET_OBJECT_NAME( shader, name, VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT );"
"%bs%" %outfb% "}"

"%bs%" %outfb% "void vk_bind_generated_pixel_shaders( void ){"

@rem compile generic shader variations from templates
@rem vertex shader
@rem for /L %%i in ( 0,1,2 ) do (                		@rem vbo 
@rem    call :compile_refraction_vertex_shader %%i

@rem 	for /L %%j in ( 0,1,1 ) do (                @rem fastlight 
@rem		for /L %%k in ( 0,1,3 ) do (                @rem light 
@rem			for /L %%l in ( 0,1,2 ) do (            @rem tx   
@rem				for /L %%m in ( 0,1,1 ) do (        @rem +env
@rem					for /L %%n in ( 0,1,1 ) do (    @rem +fog
@rem						call :compile_vertex_shader %%i, %%j, %%k, %%l, %%m, %%n
@rem					)
@rem				)
@rem			)
@rem		)
	...
	for /L %%j in ( 0,1,3 ) do (                @rem light
		for /L %%k in ( 0,1,2 ) do (            @rem tx 
			for /L %%l in ( 0,1,1 ) do (        @rem +fog
				call :compile_fragment_shader 0, %%j, %%k, %%l
			)
		)
	)
@rem )
...
:compile_fragment_shader
	...
	"%bs%" %outfb% "    vk_set_shader_name_pixel( vk.shaders.frag.gen[%1][%2][%3][0][%4], ""frag_%name%"" );"
    ...
    if %3 equ 0 goto continue
		...
		"%bs%" %outfb% "    vk_set_shader_name_pixel( vk.shaders.frag.gen[%1][%2][%3][1][%4], ""frag_!fastlight_id[%1]!!light_id[%2]!!tx_id[%3]!_!cl_id[%3]!!fog_id[%4]!"" );"
    :continue
exit /B