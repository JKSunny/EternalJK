...
static void compile_and_convert_template_shaders( void )
{
    ...
    // identity & depth fragment shaders
    for ( i = 0; i < 2; ++i ) // bindless
    { 
        std::string is_bindless = std::string(bindless_ids[i]);

        defines = join_flags({ "-DUSE_CL0_IDENT ", bindless_flags[i] });
        name    = "vert_" + is_bindless + "tx0_ident";
        ids     = join_indexes("vk.shaders.vert.gen0_ident", { i });

        create_shader_task( "gen_vert.tmpl", "vert", name.c_str(), ids.c_str(), defines.c_str(), i == 1 );

        ids     = join_indexes("vk.shaders.frag.gen0_ident", { i });
        name    = "frag_" + is_bindless + "tx0_ident";
        create_shader_task( "gen_frag.tmpl", "frag", name.c_str(), ids.c_str(), defines.c_str(), i == 1 );

        defines = join_flags({ "-DUSE_ATEST -DUSE_DF ", bindless_flags[i] });
        name    = "frag_" + is_bindless + "tx0_df";
        ids     = join_indexes("vk.shaders.frag.gen0_df", { i });
        create_shader_task( "gen_frag.tmpl", "frag", name.c_str(), ids.c_str(), defines.c_str(), i == 1 );
    }  
    ...
    for ( i = 0; i < 2; ++i ) { // bindless
        for ( j = 0; j < 3; ++j ) { // vbo
            // vertex
            defines = join_flags({ bindless_flags[i], vbo_flags[j] });
            name    = "vert_refraction_" + std::string(bindless_ids[i]) + vbo_ids[j];
            ids     = join_indexes("vk.shaders.refraction_vs", { i, j });

            create_shader_task( "refraction_vert.tmpl", "vert", name.c_str(), ids.c_str(), defines.c_str(), i == 1);
        }

        // fragment
        defines = join_flags({ bindless_flags[i] });
        name    = "frag_refraction_" + std::string(bindless_ids[i]);
        ids     = join_indexes("vk.shaders.refraction_fs", { i });

        create_shader_task( "refraction_frag.tmpl", "frag", name.c_str(), ids.c_str(), defines.c_str(), i == 1);
    }
    ...
    for ( i = 0; i < 2; ++i ) { // bindless
        for ( j = 0; j < 2; ++j ) { // fastlight
            for ( k = 0; k < 2; ++k ) { // pixel
                for ( l = 0; l < 3; ++l ) { // tx
                    for ( m = 0; m < 2; ++m ) { // fog
                        defines = join_flags({ bindless_flags[i], fastlight_flags[j], light_flags[k], tx_flags[l], fog_flags[m] });

                        if ( l == 0 ) 
                            defines += " -DUSE_ATEST";

                        name    = "frag_" + std::string(bindless_ids[i]) + fastlight_ids[j] + light_ids[k] + tx_ids[l] + fog_ids[m];
                        ids     = join_indexes("vk.shaders.frag.gen", { i, j, k, l, 0, m });

                        create_shader_task("gen_frag.tmpl", "frag", name.c_str(), ids.c_str(), defines.c_str(), i == 1);

                        if (l != 0) {
                            defines_cl  = join_flags({ bindless_flags[i], fastlight_flags[j], light_flags[k], tx_flags[l], cl_flags[l], fog_flags[m] });
                            name_cl     = "frag_" + std::string(bindless_ids[i]) + fastlight_ids[j] + light_ids[k] + tx_ids[l] + "_" + cl_ids[l] + fog_ids[m];
                            ids_cl      = join_indexes("vk.shaders.frag.gen", { i, j, k, l, 1, m });

                            create_shader_task("gen_frag.tmpl", "frag", name_cl.c_str(), ids_cl.c_str(), defines_cl.c_str(), i == 1);
                        }
                    }
                }
            }
            ...
            for ( k = 0; k < 3; ++k ) { // vbo
                for ( l = 0; l < 2; ++l ) { // light
                    for ( m = 0; m < 3; ++m ) { // tx
                        for ( n = 0; n < 2; ++n ) { // env
                            for ( o = 0; o < 2; ++o ) { // fog
                                defines = join_flags({ bindless_flags[i], fastlight_flags[j], vbo_flags[k], light_flags[l], tx_flags[m], env_flags[n], fog_flags[o] });
                                name    = "vert_" + std::string(bindless_ids[i]) + fastlight_ids[j] + vbo_ids[k] + light_ids[l] + tx_ids[m] + env_ids[n] + fog_ids[o];
                                ids     = join_indexes("vk.shaders.vert.gen", { i, j, k, l, m, 0, n, o });

                                create_shader_task("gen_vert.tmpl", "vert", name.c_str(), ids.c_str(), defines.c_str(), i == 1);

                                if (m != 0) {
                                    defines_cl  = join_flags({ bindless_flags[i], fastlight_flags[j], vbo_flags[k], light_flags[l], tx_flags[m], cl_flags[m], env_flags[n], fog_flags[o] });
                                    name_cl     = "vert_" + std::string(bindless_ids[i]) + fastlight_ids[j] + vbo_ids[k] + light_ids[l] + tx_ids[m] + "_" + cl_ids[m] + env_ids[n] + fog_ids[o];
                                    ids_cl      = join_indexes("vk.shaders.vert.gen", { i, j, k, l, m, 1, n, o });

                                    create_shader_task("gen_vert.tmpl", "vert", name_cl.c_str(), ids_cl.c_str(), defines_cl.c_str(), i == 1);
                                }
                            }
                        }
                    }
...
static void write_all_shaders_to_file( const char *out_file ) 
{
    ...
    for ( i = 0; i < task_count; ++i )
    {
        ...
        for ( j = 0; j < output->size; ++j )
        {
            ...
            // comma if it's not the last byte
            if ( size < output->size )
                fputs(", ", f_out);

            // newline every `line_length` bytes
